...
import { BigIntFromString, BooleanFromString, fromNullable, fromRefinement, NumberFromString, } from 'io-ts-types';
...
export const UInt8 = numberFromStringConverter(fromRefinement('UInt8', (n) => t.number.is(n) && Number.isInteger(n) && n >= 0 && n < 256));
export const Int8 = numberFromStringConverter(fromRefinement('Int8', (n) => t.number.is(n) && Number.isInteger(n) && n >= -128 && n < 128));
export const UInt16 = numberFromStringConverter(fromRefinement('UInt16', (n) => t.number.is(n) && Number.isInteger(n) && n >= 0 && n < 0x10000));
export const Int16 = numberFromStringConverter(fromRefinement('Int16', (n) => t.number.is(n) && Number.isInteger(n) && n >= -32768 && n < 32768));
export const UInt32 = numberFromStringConverter(fromRefinement('UInt32', (n) => t.number.is(n) && Number.isInteger(n) && n >= 0 && n <= 4294967295));
export const Int32 = numberFromStringConverter(fromRefinement('Int32', (n) => t.number.is(n) && Number.isInteger(n) && n >= -2147483648 && n < 2147483648));
...
*/
export function withDefault(codec, defaultValue) {
    let value = defaultValue;
    if (!codec.is(value)) {
        const validation = codec.decode(value);
        if (isLeft(validation))
            throw new TypeError(PathReporter.report(validation).join('\n'));
        value = validation.right;
    }
    return fromNullable(codec, value);
}
export class BufferFromBase64 extends t.Type {
