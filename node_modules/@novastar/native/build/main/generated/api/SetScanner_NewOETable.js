...
function createSetScanner_NewOETable(addr, portAddr, scanBoardAddr, bBroadcast, newOETable) {
...
exports.default = createSetScanner_NewOETable;
codec_1.Session.prototype.SetScanner_NewOETable = async function SetScanner_NewOETable(addr, portAddr, scanBoardAddr, bBroadcast, newOETable) {
    const req = createSetScanner_NewOETable(addr, portAddr, scanBoardAddr, bBroadcast, newOETable);
    await this.connection.send(req);
};
codec_1.Session.prototype.trySetScanner_NewOETable = async function trySetScanner_NewOETable(addr, portAddr, scanBoardAddr, newOETable) {
    const req = createSetScanner_NewOETable(addr, portAddr, scanBoardAddr, false, newOETable);
    return (await this.connection.trySend(req))?.ack ?? null;
};
//# sourceMappingURL=SetScanner_NewOETable.js.map