"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const codec_1 = require("@novastar/codec");
const AddressMapping_1 = __importDefault(require("../AddressMapping"));
function createSetBrightnessModel(addr, portAddr, scanBoardAddr, bBroadcast, brightnessmodel) {
    const $data = (0, codec_1.encodeUIntLE)(brightnessmodel, AddressMapping_1.default.BrightnessModelOccupancy);
    const req = new codec_1.Request($data, bBroadcast, 'SetBrightnessModel');
    req.destination = addr;
    req.deviceType = 1;
    req.port = portAddr;
    req.rcvIndex = scanBoardAddr;
    req.address = AddressMapping_1.default.BrightnessModelAddr;
    return req;
}
exports.default = createSetBrightnessModel;
codec_1.Session.prototype.SetBrightnessModel = async function SetBrightnessModel(addr, portAddr, scanBoardAddr, bBroadcast, brightnessmodel) {
    const req = createSetBrightnessModel(addr, portAddr, scanBoardAddr, bBroadcast, brightnessmodel);
    await this.connection.send(req);
};
codec_1.Session.prototype.trySetBrightnessModel = async function trySetBrightnessModel(addr, portAddr, scanBoardAddr, brightnessmodel) {
    const req = createSetBrightnessModel(addr, portAddr, scanBoardAddr, false, brightnessmodel);
    return (await this.connection.trySend(req))?.ack ?? null;
};
//# sourceMappingURL=SetBrightnessModel.js.map