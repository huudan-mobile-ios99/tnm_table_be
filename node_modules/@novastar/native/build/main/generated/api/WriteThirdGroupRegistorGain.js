...
function createWriteThirdGroupRegistorGain(addr, portAddr, scanBoardAddr, bBroadcast, data) {
...
exports.default = createWriteThirdGroupRegistorGain;
codec_1.Session.prototype.WriteThirdGroupRegistorGain = async function WriteThirdGroupRegistorGain(addr, portAddr, scanBoardAddr, bBroadcast, data) {
    const req = createWriteThirdGroupRegistorGain(addr, portAddr, scanBoardAddr, bBroadcast, data);
    await this.connection.send(req);
};
codec_1.Session.prototype.tryWriteThirdGroupRegistorGain = async function tryWriteThirdGroupRegistorGain(addr, portAddr, scanBoardAddr, data) {
    const req = createWriteThirdGroupRegistorGain(addr, portAddr, scanBoardAddr, false, data);
    return (await this.connection.trySend(req))?.ack ?? null;
};
//# sourceMappingURL=WriteThirdGroupRegistorGain.js.map