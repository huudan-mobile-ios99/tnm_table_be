...
import { CaculateShiftUnitNum, GetGclkInfoByPartNumPerRef, GetGrayScaleByGclkNumPerRef, } from './CommonCalculator';
...
export default class ScannerCapabilityICN2053 extends ScannerCapabilitySUM2030 {
    UpdateParameters(scanBdProperty, vsFreq) {
        ...
        const { ModulePixelCols, ModulePixelRows, TotalPointInTable, DataDirectType = DataDirectionTypeEnum.Horizontal, ScanType, DriverChipType: chipType, } = StandardLedModuleProp;
        if (!chipType)
            throw new TypeError('Unknown chipType');
        if (!IsIrRegular) {
            this._shiftNumPerSubField = CaculateShiftUnitNum(ModulePixelCols, ModulePixelRows, TotalPointInTable, DataDirectType, Width, Height);
        ...
        scanBdProperty.GrayDepth = GetGrayScaleByGclkNumPerRef(this._gclkNumPerRef -
    ...
    CaculateAllParameters(chipType, partNumPerRef, blackTime) {
        ...
        const { gclkNumPerScan, validGclkNumPerScan, brightGclkNumPerScan } = GetGclkInfoByPartNumPerRef(chipType, partNumPerRef);
