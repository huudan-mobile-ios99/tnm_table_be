...
import { isComplexScreen, isSimpleScreen, isStandardScreen } from './common';
/**
 ...
 * @param index
 */
export default function convertLEDDisplayInfoToScreenDataInSoftSpace(scr, index) {
    const UUID = uuid();
    if (isSimpleScreen(scr)) {
        return makeStruct(ScreenDataInSoftSpace, {
            ...
            OnePortLoadInfo: scr.PortScanBdInfoList?.map(port => makeStruct(OnePortLoadInfo, {
    ...
    if (isStandardScreen(scr)) {
        ...
        return makeStruct(ScreenDataInSoftSpace, {
            ...
            CabinetInDevice: scr.ScannerRegionList?.map(region => makeStruct(CabinetInDevice, {
    ...
    if (isComplexScreen(scr)) {
        return makeStruct(ScreenDataInSoftSpace, {
            ...
            VirMode: scr.VirtualMode,
            CabinetInDevice: scr.ScanBoardRegionInfoList?.map(region => makeStruct(CabinetInDevice, {
                DevID: region.SenderIndex,
                NetPort: region.PortIndex,
                ConnectID: region.ConnectIndex,
                XPos: region.X,
