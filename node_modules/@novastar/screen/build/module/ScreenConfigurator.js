import { 
// Connection,
decodeUIntLE, delay, DeviceType, ErrorType, Request, series, } from '@novastar/codec';
...
import { FILE_COMPRESS_AREA_START_ADDR, FileInfoObjectList, ScreenFileType, ScreenFileVersion, } from './FileInfo';
...
import { decodeModulationInfo, encodeModulationInfo, ModulationInfoHeader, SenderModulationFlag, } from './ModulationInfo';
import { decodeRedundancyInfo, encodeRedundancyInfo, ReduFlag, RedundancyInfo, } from './RedundancyInfo';
import { ScreenConfigInfo } from './ScreenConfigInfo';
import Session from './Session';
import { ParamSize, SoftwareSpaceHeader, SoftwareSpaceHeaderFlag, SoftwareSpaceHeaderVersion, } from './SoftwareSpaceHeader';
import { crc16, crc8, isHorizontalConnection, isLeftConnection, isSimpleScreen, isTopConnection, itFirstNotNull, minimax, pack, unpack, } from './common';
...
const consolidateResults = (results) => results.every(res => res == null) ? null : results.every(res => res === ErrorType.Succeeded);
...
export default class ScreenConfigurator {
     ...
     */
    reload = this.queue(this.reloadImpl);
    /**
     * Save screen configurations to devices.
     */
    save = this.queue(this.saveImpl);
    ReadHWStatus = this.createReadGenerator('ReadAllStatus', res => new HWStatus(res.data).toJSON());
    ReadReceivingCardMCURemarks = this.createReadGenerator('ReadScanner_McuProgramRemarks', stringConverter);
    ReadReceivingCardFPGARemarks = this.createReadGenerator('ReadScanner_FPGAProgramRemarks', stringConverter);
    ReadChipType = this.createReadGenerator('ReadDriverType', decodeUIntLE);
    ReadFirstChipType = firstCreator(this.ReadChipType);
    WriteBrightness = this.createWriter('SetGlobalBrightness', percent => Math.ceil((minimax(0, 100, percent) * 255) / 100));
    ReadBrightness = this.createReadGenerator('ReadGlobalBrightness', res => Math.round((decodeUIntLE(res) * 10000) / 255) / 100);
    ...
    #screenVersion = 0;
    constructor(arg) {
        this.session = (arg instanceof Connection ? new Session(arg) : arg);
    ...
    GetScreenAllPort(screen, toRead = false) {
        ...
        const list = GetScreenPortAddrInfo(scr).flatMap(({ SenderIndex, PortIndex, MinConnectIndex, LoadScannerCount }) => {
    ...
    createWriter(name, encoder = identity) {
        return {
            async [name](value, screen = -1) {
                ...
                const results = await series(addresses, ({ SenderIndex, PortIndex, ScanIndex }) => this.session[`try${name}`](SenderIndex, PortIndex, ScanIndex, val));
                return consolidateResults(results);
            },
        }[name].bind(this);
    }
    ...
    async reloadImpl() {
        ...
        await series(this.devices, async ({ maxPackageSize, model }, index) => {
            if (!IsSystemController(model))
                return;
    ...
    async saveImpl() {
        ...
        const items = splitScreensByDevice(this.screens.map(convertLEDDisplayInfoToScreenDataInSoftSpace));
    ...
    async saveRedundancy(index) {
        ...
        await this.session.SetSender_SoftwareSpace(index, false, data, data.length, SoftwareSpaceBaseAddress.REDUNDANCY_BASE_ADDRESS);
    }
    ...
    async sendParametersToScanBoardGroupImpl(scanBdProperty, isSmartMode = false, isSmartNoSend = true, senderIndex = 255, portIndex = 255, scanBdIndex = 0xffff) {
        if (!isValidScanBdProp(scanBdProperty))
        ...
        if (!isSmartMode) {
            await this.session.SetSelfTestMode(senderIndex, portIndex, scanBdIndex, false, TestModeEnum.ParaFreeze);
            await delay(1000);
        }
        ...
        const autoRefreshRates = GetAutoRefreshRateBytesSeq(SetVariousScanBdRefreshRate(scanBdProperty, isSmartMode));
    ...
    async readRedundancy(index) {
        const info = new RedundancyInfo(await this.session.ReadSender_SoftwareSpace(index, RedundancyInfo.baseSize, SoftwareSpaceBaseAddress.REDUNDANCY_BASE_ADDRESS));
        debug(`redundancyHeader: ${info.header} (${info.length})`);
        if (info.header !== ReduFlag || info.length === 0)
            return;
        const data = await this.session.ReadSender_SoftwareSpace(index, RedundancyInfo.baseSize + info.length, SoftwareSpaceBaseAddress.REDUNDANCY_BASE_ADDRESS);
        this.#reduList = decodeRedundancyInfo(data);
        debug(`redundancy: ${JSON.stringify(this.reduList)}`);
    }
    async readModulation(index = 0) {
        const data = await this.session.ReadSender_SoftwareSpace(index, ModulationInfoHeader.baseSize, SoftwareSpaceBaseAddress.MODULATION_BASE_ADDRESS);
        const header = new ModulationInfoHeader(data);
        if (header.header !== SenderModulationFlag || header.length === 0)
            return;
        const total = header.length + ModulationInfoHeader.baseSize;
        const buf = await this.session.ReadSender_SoftwareSpace(0, total, SoftwareSpaceBaseAddress.MODULATION_BASE_ADDRESS);
    ...
    async saveModulation(index) {
        ...
        await this.session.SetSender_SoftwareSpace(index, false, data, data.length, SoftwareSpaceBaseAddress.MODULATION_BASE_ADDRESS);
    }
    ...
    async SetScreenALLWidth() {
        const results = await series(this.screens, scr => {
            ...
            return series(senders, ({ SenderIndex }) => this.session.trySetSenderVideoEnclosingMode(SenderIndex, location.rightBottom.x, location.rightBottom.y));
        });
    ...
    async ReadDviScreenInfo(header, index) {
        ...
        const data = await this.session.ReadSender_SoftwareSpace(index, DviScreenConfigInfo.baseSize + dviInfoLength + screenInfoLength + adjustInfoLength, SoftwareSpaceBaseAddress.BASE_ADDRESS);
    ...
    async ReadSenderScreenConfigInfo(index) {
        ...
        if (!res)
            return;
        const { Type, ScanBdCols, ScanBdRows, CabinetsPerPort, PixelColsInScanBd, PixelRowsInScanBd, ConnectType, X, Y, } = new ScreenConfigInfo(res.data);
        switch (Type) {
            case 1: {
                ...
                if (isHorizontalConnection(ConnectType)) {
                    portCols = 1;
                    portRows = portCount;
                    const getRow = (row) => minimax(0, ScanBdRows, isTopConnection(ConnectType) ? row : ScanBdRows - row);
                    const rowsPerPort = CabinetsPerPort / ScanBdCols;
                    ports = range(portCount).map(PortIndex => {
                        const extremes = [
                    ...
                    portCols = portCount;
                    portRows = 1;
                    const getCol = (col) => minimax(0, ScanBdCols, isLeftConnection(ConnectType) ? col : ScanBdCols - col);
    ...
    async ReadSoftSpaceData(spaceHeader, index) {
        ...
        const fileInfoListV = FileInfoObjectList.decode(JSON.parse(await unpack(paramData.slice(2), ss.fileInfoSize, fileInfoCompressData)));
        ...
        const screens = (await series(fileInfoList.SectionFormat, async (info) => {
