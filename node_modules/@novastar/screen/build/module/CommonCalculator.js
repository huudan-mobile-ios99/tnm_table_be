...
export function CaculateShiftUnitNum(modWidth, modHeight, modTotalPointInTable, directionType, cabinetRealWidth, cabinetRealHeight) {
    return ((directionType !== DataDirectionTypeEnum.Horizontal
        ? cabinetRealHeight / modHeight
        : cabinetRealWidth / modWidth) * modTotalPointInTable);
}
export const GetGrayScaleByGclkNumPerRef = (gclkNumPerRef) => {
...
export function GetGclkInfoByPartNumPerRef(chipType, partNumPerRef, isPWMModel = true, isGLCK = true) {
    switch (chipType) {
...
export const isValidScanBdProp = hasProps('IsDExtendMode', 'TwentyDataGroupEnable', 'Is24DataGroup', 'Is28DataGroup', 'StandardLedModuleProp', 'IsSymmetricalOutputMode', 'CommonIrCabinetMode', 'IsIrRegular', 'ChipPropey', 'SpecialFrameRate');
export const isValidStandardLedModuleProp = hasProps('ScreenDriveType');
export const GetDoorCountFromScanBdInfo = (scanBdProp) => {
...
export const GetMaxCabinetDataGroup = (scanBdProp) => {
    if (!isValidScanBdProp(scanBdProp))
        throw new TypeError('Invalid ScanBoardProperty');
...
export const GetHorizontalCascadeMaxLoad = (scanBdProp, maxLoadedPixels, maxArea) => {
    if (!isValidScanBdProp(scanBdProp))
        throw new TypeError('Invalid ScanBoardProperty');
...
export const GetVerticalCascadeMaxLoad = (scanBdProp, maxLoadedPixels, maxArea) => {
    if (!isValidScanBdProp(scanBdProp))
        throw new TypeError('Invalid ScanBoardProperty');
...
export const CaculateMaxloadSize = (scanBdProp, maxLoadedPixels, maxArea) => {
    switch (scanBdProp.ModCascadeType) {
        case ModuleCascadeDiretionEnum.RightLeft:
        case ModuleCascadeDiretionEnum.LeftRight: {
            const { maxWidth, ...other } = GetHorizontalCascadeMaxLoad(scanBdProp, maxLoadedPixels, maxArea);
            return {
        ...
        case ModuleCascadeDiretionEnum.UpDown: {
            const { maxHeight, ...other } = GetVerticalCascadeMaxLoad(scanBdProp, maxLoadedPixels, maxArea);
            return {
...
export const CaculateCutMapAddrIndex = (width, cutRect, pointBytes) => {
