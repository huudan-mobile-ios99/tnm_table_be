...
const DataGroupSequenceStruct = new typed_struct_1.default('DataGroupSequence')
    .Bits32({
    a: [0, 4],
    b: [4, 4],
    c: [8, 4],
...
const getBool = (trueValue = 1) => (_, [data]) => data === trueValue;
const setBool = (trueValue = 1, falseValue = 0) => (_, buffer, value) => buffer.writeUInt8(value ? trueValue : falseValue) > 0;
const ghostGetter = (type, buffer) => {
    switch (type) {
...
const ghostSetter = (type, buffer, value) => {
    switch (type) {
        case 'IsCloseGhostSignal': {
            const mask = 1 << 5;
            if (value)
 ...
 */
exports.ScanboardData = new typed_struct_1.default('ScanboardData')
    .UInt8('GammaValue')
    .UInt8('Brightness')
    .UInt8('RedBright')
    .UInt8('GreenBright')
    ...
    .UInt8('DecType')
    .UInt8('DataGroup')
    .Custom('DataGroupSequence', DataGroupSequenceStruct.baseSize, getDataGroupSequence, setDataGroupSequence)
    .UInt8('ModuleCols')
    .UInt8('ModuleRows')
    .UInt16LE('Width')
    .UInt16LE('Height')
    ...
    .Custom('Is28DataGroup', 1, (_, [data]) => data === 18, (_, buffer, value) => buffer.writeUInt8(value ? 18 : 0) > 0)
    .back()
    .Custom('Is24DataGroup', 1, (_, [data]) => data === 2, (_, buffer, value) => buffer.writeUInt8(value ? 2 : 0) > 0)
    .back()
    .Custom('IsDExtendMode', 1, (_, [data]) => data === 1, (_, buffer, value) => buffer.writeUInt8(value ? 1 : 0) > 0)
    .UInt8('GrayDepth')
    .Bits8({
    GrayMode: [0, 4],
    GrayRealize: [4, 4],
})
    .Custom('RefNumPerVs', 2, (_, [l, h]) => l + (h << 7), (_, buffer, value) => {
    ...
    .UInt8('GCLKDuty')
    .UInt8('SubFields')
    .Buffer('SubFieldPart', MaxValue_1.default.MAX_SUBFIELDPART_COUNT)
    .Buffer('RowsCtrlByDataGroup', MaxValue_1.default.MAX_MODULEDATAGROUP)
    ...
    .UInt8('OfflineFrame')
    .Buffer('ScanABCDCode', MaxValue_1.default.MAX_SCAN >>> 1)
    ...
    .Buffer('StartPositionOfDataGroup', MaxValue_1.default.MAX_MODULEDATAGROUP)
    .seek(2)
    .Buffer('Gain1', 8)
    ...
    .seek(1)
    .Custom('CabinetRotateAngle', 1, (_, [data]) => ((data + 3) % 4), (_, buffer, value) => buffer.writeUInt8((value + 1) % 4) > 0)
    ...
    .Buffer('GroupSwapInfo', MaxValue_1.default.MAX_SCANNER_DATAGROUP) // 184
    .Bits8({
    ...
    .UInt8('chipGrayBit')
    .back(0)
    .seek(MaxValueInfo_1.default.MAX_PARAMETER_TABLE_LEN)
    .compile();
const hasEnhancedMode = (0, common_1.hasProps)('EnhancedMode', 'EnhancedModeSwitch');
const is2033 = (0, common_1.hasProps)('UsingBright', 'UsingSUM2033Gamma');
const isChip2053ExtendPropety = (chipPropey) => chipPropey['@_xsi:type'] === 'Chip2053ExtendPropety';
const isChip2163ExtendPropety = (chipPropey) => chipPropey['@_xsi:type'] === 'Chip2163ExtendPropety';
const encodeScanBoardProperty = (scanBdProp) => {
    if (!(0, CommonCalculator_1.isValidScanBdProp)(scanBdProp))
        throw new TypeError('Invalid ScanBoardProperty');
    const scanData = new exports.ScanboardData();
    Object.freeze(scanData);
    const { ChipPropey, StandardLedModuleProp: { DriverChipType }, MaxGammaValue, LS9960Data, } = scanBdProp;
    ...
    if ([
        ChipType_1.ChipTypeEnum.Chip_ICN2053,
        ChipType_1.ChipTypeEnum.Chip_ICN2050,
        ChipType_1.ChipTypeEnum.Chip_ICND2055,
        ChipType_1.ChipTypeEnum.Chip_ICN2065,
        ChipType_1.ChipTypeEnum.Chip_VOD5153,
        ChipType_1.ChipTypeEnum.Chip_ICND2163,
        ChipType_1.ChipTypeEnum.Chip_ICND2200,
        ChipType_1.ChipTypeEnum.Chip_ICND2150,
        ChipType_1.ChipTypeEnum.Chip_ICND2153,
    ].includes(DriverChipType)) {
        scanData.MaxGammaValue = MaxGammaValue;
    }
    else if (DriverChipType === ChipType_1.ChipTypeEnum.Chip_LS9960 && LS9960Data?.MaxGamaValue) {
    ...
    // }
    const { SpecialFrameRate: { StartRefNumPerVs, StartSubFields, EndRefNumPerVs, EndSubFields }, StandardLedModuleProp: { ScreenDriveType, DataDirectType, ModulePixelCols, ModulePixelRows, DecodeTypePro, DriverTypePro, ScanType, OEPolarity, DecType, DataGroup, DataGroupSequence, TotalPointInTable, LineBias, ScanABCDCode, // *
    RowsCtrlByDataGroup, // A
    RGBCode, SerialColorNum, SerialDotsNumPerColor, SerialRGBCode, ChannelEnableData, // A
    // DecType
    ChipNumber, }, GammaValue, Brightness, RedBright, GreenBright, BlueBright, VRedBright, MonitorRGBCode, DclkHighRatio, ModuleCols, ModuleRows, Width, Height, PhysicalDataGroupNum, ModCascadeType, PointNumberPerDriver, // *
    IsIrRegular, CommonIrCabinetMode, LogicalDataGroupNum, DriverFucntion, IsDExtendMode, Is24DataGroup, Is28DataGroup, 
    // GrayDepth,
    // chipGrayBit,
    GrayRealize, GrayMode, RefNumPerVs, BlankUnitNumPerScan, RowChangePoint, ABCDRollOver, IsReverseScanOutput, ScanSequenceAdjustEn, GclkNumPerScan, LightTimeRatio, ShiftUnitNum, // *
    TotalUnitNum, LightTime, DclkUnitCycle, DclkPhase, DclkHigh, GCLKRate, GCLKPhase, GCLKDuty, SubFields, SubFieldPart, // A
    TotalGclkUnitNumPerScan, // *
    LatDelay, // *
    IsEnableCalibration, CorrectionMode, IsChromaCorrentionLowGray, CoefSourceType, OfflineFrame, 
    // MaxGammaValue, // *
    LightTimeNum2, // *
    LineScanTime, IsSymmetricalOutputMode, IsOpenClearBlankLine, LowAshCompensationOne, LowAshCompensationTwo, IsOpenLowAshCompensation, PowerOnBrightnessAdjustEn, OpenEMCFun, EnableEnhanceFirstScan, IsOpenSetGainValue, FirstScanCompensation, OtherScanCompenscation, IsEnableOtherRefreshNumParams, IsSupportHighLoad, 
    // startPositionOfDataGroup,
    IsCloseGhostSignal, IsGhostSignalNegation, SerialDecodeRepeatTimes, 
    // IsSupportMBI5041BNewTiming,
    // IsSupportDriverInterDeghostEnable
    LightFlashClose, CabinetRotateAngle, 
    // OperatingMode, GrayEnhanced
    RedNoCorrectionThreshold, GreenNoCorrectionThreshold, BlueNoCorrectionThreshold, VirRedNoCorrectionThreshold, RedNoCorrectionAttenuation, GreenNoCorrectionAttenuation, BlueNoCorrectionAttenuation, VirRedNoCorrectionAttenuation, CtrlEndPoint, IsNewOERamEnable, LowGrayQuery, LowGrayCompensation, AddrExtend, TwentyDataGroupEnable, GroupSwapEnable, GroupSwapInfo, // A
    IsLowGrayRamEnable, GrayDepth, // *
    PreChargeTime, DataGroupOutPutType, // *
    IsStarSwipPoint, UnitIcCount, MBI515xDeltaT, MBI515xDhT, LogicalShiftUnitNum, 
    // chipRegistorConfig
    RealBrightnessOf2053, TotalLightCdfResault, ReduceHighContrast, CurrentPosition, CurrentPhase, TinyAfterglowMoment, TinyLineFeedMoment, BrightPriorityMode, ABCSignalDelayEnable, DESignalDelayEnable, ABCDESignalDelayTime, OddEvenScan, 
    // Support22BitModel (BitModel)
    RealPhysicalGroupNum, AutoOrManual5252ResetSwitch, 
    // Auto5252ResetTime: { Hour, Minute },
    IntervalDate5252, EnableDirectMode, 
    // EliminateMode,
    // EliminatePotential,
    ...
    return exports.ScanboardData.raw(scanData);
};
exports.encodeScanBoardProperty = encodeScanBoardProperty;
//# sourceMappingURL=ScanboardData.js.map